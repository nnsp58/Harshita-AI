...
export default function DocumentEditorPanel() {
  const { currentDocument, setCurrentDocument, setResponseMode, subscriptionMode } = useStore();
  ...
  const handlePrint = () => {
    ...
    const watermarkHtml = isPremium ? '' : `
      <div style="position:fixed; top:50%; left:50%; transform:translate(-50%, -50%) rotate(-30deg); opacity:0.06; width:60%; height:auto; pointer-events:none; user-select:none; z-index:0; display:flex; flex-direction:column; align-items:center; justify-content:center;">
        <div style="font-size:150pt; font-weight:900; line-height:1; color:#000; font-family:serif; letter-spacing:-0.05em;">HA</div>
        <div style="margin-top:20px; text-align:center; opacity:0.3; font-size:10pt; color:#000; font-weight:600; font-family:sans-serif;">
  ...
  const downloadDocx = () => {
    ...
    const sourceHTML = header + watermarkHtml + `<div style="position:relative; z-index:1;">` + editorRef.current.innerHTML + `</div>` + footer;
  ...
  return (
    <div className="h-full flex flex-col bg-[#e5e7eb] font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden relative p-8 flex justify-center">
        {/* A4 Paper container */}
        <div 
          className="bg-white shadow-2xl transition-transform origin-top relative overflow-hidden"
          style={{ 
            width: '210mm', 
            minHeight: '297mm',
            padding: '25mm',
        ...
        >
          ...
          <div 
            ...
            contentEditable
            onInput={handleInput}
            className="w-full h-full outline-none prose prose-sm max-w-none prose-p:leading-relaxed prose-headings:text-center text-black relative z-10"
            style={{ 
